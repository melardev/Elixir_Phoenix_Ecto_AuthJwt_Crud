...
# Introduction
Phoenix api application with JWT protected CRUD using Ecto.

Start the application:
* Install dependencies with `mix deps.get`
* Configure the database(at this moment it uses Postgres with postgres:password credentials)
* Create and migrate your database with `mix ecto.setup`
* Start Phoenix endpoint with `mix phx.server`
* Open [`localhost:4000`](http://localhost:4000)

## Steps
`mix phx.new api_phoenix_jwt_crud --no-webpack --no-html`
- Change DB credentials then
`mix ecto.create`
`mix phx.gen.json Todos Todo todos title:string description:string completed:boolean`
- Update migration then
`mix ecto.migrate`
- Generate a JSON API
`mix phx.gen.json Todos Todo todos title:string description:string completed:boolean`
- Generate User Model
`mix phx.gen.json Auth User users username:string:unique email:string:unique hashed_password:string`
